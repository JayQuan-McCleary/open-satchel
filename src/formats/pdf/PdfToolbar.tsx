...
export default function PdfToolbar({ tabId }: FormatViewerProps) {
  ...
  return (
    <div style={{ background: 'var(--bg-primary)', borderBottom: '1px solid var(--border)', height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      {/* Ribbon content. Flex-wraps to a second row on narrow windows instead
          of scrolling horizontally. The grid row containing the ribbon uses
          auto-height so wrapped content stays visible. */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 0, flex: 1, padding: '0 4px', flexWrap: 'wrap', overflowX: 'hidden', overflowY: 'visible' }}>
        {ribbonTab === 'Home' && (
          <>
