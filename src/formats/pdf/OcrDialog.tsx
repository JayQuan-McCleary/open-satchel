  ...
  const runOcr = useCallback(async () => {
    ...
    try {
      ...
      for (let i = 0; i < pagesToProcess.length; i++) {
        ...
        // tesseract.js v5+ renamed Page.words access — the field exists at
        // runtime but isn't on the typed `Page` anymore.
        const words0 = (result.data as { words?: Array<{ text: string; bbox: { x0: number; y0: number; x1: number; y1: number } }> }).words
        if (outputMode === 'searchable' && words0) {
          const words = words0.map((w) => ({
            text: w.text,
